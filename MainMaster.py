...
conn = mysql.connector.connect(
   user='root', password='1234567890', host='localhost', database='cs_project')
...
def register():
    ...
    if name=='' or phnum==''or nationality=='' or Address==''or typeroom1==''or Typeroom1=='' or checkroom=='' or checkin==''or checkout=='' or checktype==''or checktypech==''or roomnum==''or reservation==''or price0==''or price1==''or price2==''or More=='' :
       ...
       try:
           ...
           conn.rollback()
       message.set("บันทึกข้อมูลสำเร็จ")
...
def openWindow():
    ...
    lbTitle = Label(mainWindow, text="บันทึกรายการ", fg="black",height=2, width=15, font=("4711_AtNoon_Regular", 24),bg="#fdebf1").grid(row=0, column=0, sticky='NSEW', columnspan=1)

    lbcon_name = Label(mainWindow, text="ชื่อลูกค้า :", fg="black", font=("4711_AtNoon_Regular", 24), bg="#FBCFDD").grid(row=1, column=0, sticky='NSEW')
    # ----------------
    txt = StringVar()
    txtlbcon_name = Entry(mainWindow, textvariable=txt,font=("4711_AtNoon_Regular", 24)).grid(row=1, column=1, sticky='NSEW')

    lbnum = Label(mainWindow, text="เบอร์โทรศัพท์ :", fg="black", font=("4711_AtNoon_Regular", 24), bg="#FBCFDD").grid(row=1, column=2, sticky='NSEW')
    # -----------------
    txt2 = StringVar()
    txtnum = Entry(mainWindow, textvariable=txt2, font=("4711_AtNoon_Regular", 24)).grid(row=1, column=3, sticky='NSEW')

    lbNAT = Label(mainWindow, text="สัญชาติ :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=2, column=0, sticky='NSEW')
    # ----------------------
    txt3 = StringVar()
    txtNAT = Entry(mainWindow, textvariable=txt3, font=("4711_AtNoon_Regular", 24)).grid(row=2, column=1, sticky='NSEW')

    lbADD= Label(mainWindow, text="ที่อยู่ :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=2, column=2, sticky='NSEW')
    # -------------------------
    txt4 = StringVar()
    txtadd = Entry(mainWindow, textvariable=txt4, font=("4711_AtNoon_Regular", 24)).grid(row=2, column=3, sticky='NSEW')


    Label(mainWindow, text="ประเภทห้อง : ",  fg="black", font=("4711_AtNoon_Regular", 24), bg="#FBCFDD").grid(row=3, column=0, sticky='NSEW')

#  ---------------------------- radiobutton fix posittion please fix your screen
    ...
    Label(mainWindow, text="ชนิดห้องพัก" , fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD" ).grid(row=3, column=2, sticky='NSEW')
 
 
#  ---------------------------- Dropdown fix posittion please fix your screen
    ...
    Label(mainWindow, text="",textvariable=message).place(x=605,y= 432)

    lbchack_room= Label(mainWindow, text="จำนวนห้อง :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=4, column=0, sticky='NSEW')  
    # -------------------------
    txt5 = StringVar()
    txtcheck_room = Entry(mainWindow, textvariable=txt5, font=("4711_AtNoon_Regular", 24)).grid(row=4, column=1, sticky='NSEW')

    lbchack_in= Label(mainWindow, text="เข้าพักตั้งแต่วันที่ :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=4, column=2, sticky='NSEW')
    # -------------------------
    txt6 = StringVar()
    txtcheck_in = Entry(mainWindow, textvariable=txt6, font=("4711_AtNoon_Regular", 24)).grid(row=4, column=3, sticky='NSEW')

    lbchack_out= Label(mainWindow, text="ถึงวันที่ :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=5, column=0, sticky='NSEW')
    # -------------------------
    txt7 = StringVar()
    txtcheck_out = Entry(mainWindow, textvariable=txt7, font=("4711_AtNoon_Regular", 24)).grid(row=5, column=1, sticky='NSEW')

    lbcheck_type= Label(mainWindow, text="จำนวนผู้เข้าพัก ผู้ใหญ่ :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=6, column=0, sticky='NSEW')
    # -------------------------
    txt8 = StringVar()
    txtcheck_type = Entry(mainWindow, textvariable=txt8, font=("4711_AtNoon_Regular", 24)).grid(row=6, column=1, sticky='NSEW')

    lbchack_ch= Label(mainWindow, text="เด็ก :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=6, column=2, sticky='NSEW')
    # -------------------------
    txt9 = StringVar()
    txtcheck_ch = Entry(mainWindow, textvariable=txt9, font=("4711_AtNoon_Regular", 24)).grid(row=6, column=3, sticky='NSEW')

    lbchack_room_num= Label(mainWindow, text="หมายเลขห้อง :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=7, column=0, sticky='NSEW')
    # -------------------------
    txt10 = StringVar()
    txtcheck_room_num = Entry(mainWindow, textvariable=txt10, font=("4711_AtNoon_Regular", 24)).grid(row=7, column=1, sticky='NSEW')

    lbchack_re= Label(mainWindow, text="รหัสผู้จอง :", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=7, column=2, sticky='NSEW')
    # -------------------------
    txt11 = StringVar()
    txtcheck_re = Entry(mainWindow, textvariable=txt11, font=("4711_AtNoon_Regular", 24)).grid(row=7, column=3, sticky='NSEW')

    Label(mainWindow, text="ราคาห้องพัก :" , fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD" ).grid(row=8, column=0, sticky='NSEW')
 ...
 #  ---------------------------- Dropdown fix posittion please fix your screen
    ...
    dropdown1.current()
    dropdown1.place(x=233,y=426)

    Label(mainWindow, text="การชำระเงิน : ",  fg="black", font=("4711_AtNoon_Regular", 24), bg="#FBCFDD").grid(row=9, column=0, sticky='NSEW')

#-------------------------- การชำระเงิน radiobutton

    Radiobutton(mainWindow,text="ชำระแล้ว",variable=priceroom,value=1).place(x=195,y=475)
    Radiobutton(mainWindow, text="ยังไม่ชำระ", variable=priceroom, value=2).place(x=293, y=475)

    Label(mainWindow, text="เงินมัดจำ : ",  fg="black", font=("4711_AtNoon_Regular", 24), bg="#FBCFDD").grid(row=10, column=0, sticky='NSEW')

#--------------------- เงินมัดจำ radiobutton

    Radiobutton(mainWindow,text="ชำระแล้ว",variable=priceroom1,value=1).place(x=205,y=522)
    Radiobutton(mainWindow, text="ยังไม่ชำระ", variable=priceroom1, value=2).place(x=303, y=522)

    lbmore= Label(mainWindow, text="หมายเหตุ : (Y/N)", fg="black", font=("4711_AtNoon_Regular", 24),bg="#FBCFDD").grid(row=11, column=0, sticky='NSEW')
# -------------------------
    txt12 = StringVar()
...
#------------------------- Button ADDING
    ...
    btnADD = Button(mainWindow, text="Save", fg="black", font=("4711_AtNoon_Regular", 13),
                    bg="#ddfbcf", width="20", height="3", command=register).grid(row=12, column=6, sticky='NSEW')
    btnCLOSE = Button(mainWindow, text="Close", fg="black", font=("4711_AtNoon_Regular", 13),
                  bg="#fdebf1", width="20", height="3", command=exitProgram1).grid(row=12, column=3, sticky='NSEW')
